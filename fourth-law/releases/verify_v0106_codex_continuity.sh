...
for role in runtime control-room execution efficiency; do test -z "$(git -C "$WORKTREES/$role" status --porcelain)"; done
...
wait_marker(){
  ...
  for _ in $(seq 1 90); do
    ...
    echo "$state" | grep -q "$marker" && return 0
...
for role in runtime control-room execution efficiency; do
  ...
  test -z "$(git -C "$WORKTREES/$role" status --porcelain)"

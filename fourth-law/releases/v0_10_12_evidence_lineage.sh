...
REPORT=/tmp/fl-v01012-evidence-lineage.txt
STAGE="$(mktemp -d /tmp/fl-v01012-stage.XXXXXX)"
BACKUP="$(mktemp -d /opt/fl-v01012-backup.XXXXXX)"
STEP=starting
DEPLOY_STARTED=0
...
fail() {
  ...
  {
    echo FOURTH_LAW_EVIDENCE_LINEAGE_V0_10_12_FAILED
    echo "step=$STEP"
...
echo "$health" | grep -q '"version":"0.10.11"'
...
echo "control_changed=$changed" >>"$REPORT"
if test -n "$changed"; then
  unexpected="$(echo "$changed" | grep -Ev '^(app/code_workspace\.py|app/codex_control\.py|app/control_room\.py|app/evidence_lineage\.py|app/intelligence_engine\.py|app/main\.py|app/problem_engine\.py|app/scoped_verification\.py|app/shared_memory\.py|app/static/control_room\.html|tests/test_agent_budget\.py|tests/test_codex_control\.py|tests/test_codex_workspace_ui\.py|tests/test_evidence_lineage\.py|tests/test_scoped_verification\.py|tests/test_workspace_memory\.py|tests/test_human_efficiency\.py)$' || true)"
  echo "control_unexpected=$unexpected" >>"$REPORT"
  test -z "$unexpected"
else
...
grep -q '"version": "0.10.12"' "$CONTROL/app/codex_control.py"
grep -q 'class ScopedVerificationContract' "$CONTROL/app/scoped_verification.py"
grep -q 'ScopedVerificationContract(' "$CONTROL/app/intelligence_engine.py"
grep -q 'verification_contract' "$CONTROL/app/intelligence_engine.py"
grep -q 'ScopedVerificationContract(' "$CONTROL/app/problem_engine.py"
grep -q 'self.efficiency_observer.observe' "$CONTROL/app/intelligence_engine.py"
grep -q 'class EfficiencyMemoryAgent' "$CONTROL/app/shared_memory.py"
grep -q 'shared_code_memory' "$CONTROL/app/shared_memory.py"
grep -q 'INTELLIGENCE_AGENT_BUDGET.*min(12' "$CONTROL/app/main.py"
grep -q 'MAX_AGENTS = max(5, min(12' "$CONTROL/app/main.py"
grep -q '"max_agents":MAX_AGENTS' "$CONTROL/app/main.py"
grep -q 'class EvidenceLineageLedger' "$CONTROL/app/evidence_lineage.py"
grep -q '/control-room/api/evidence/{job_id}' "$CONTROL/app/control_room.py"
grep -q 'evidence_lineage' "$CONTROL/app/control_room.py"
grep -q 'Evidence / lineage' "$CONTROL/app/static/control_room.html"
grep -q 'Hashes only' "$CONTROL/app/static/control_room.html"
...
if test -n "$(runuser -u "$DEV_USER" -- git -C "$CONTROL" status --porcelain)"; then
  runuser -u "$DEV_USER" -- git -C "$CONTROL" add \
    app/code_workspace.py app/control_room.py app/evidence_lineage.py app/intelligence_engine.py app/main.py app/problem_engine.py app/scoped_verification.py app/shared_memory.py app/static/control_room.html \
    app/codex_control.py app/static/codex.html compose.yaml tests/test_codex_control.py tests/test_codex_workspace_ui.py \
    tests/test_scoped_verification.py tests/test_workspace_memory.py tests/test_human_efficiency.py tests/test_agent_budget.py tests/test_evidence_lineage.py
...
docker build --quiet -t fourth-law-agent:v0.10.12-candidate "$STAGE" >>"$REPORT" 2>&1
docker run --rm --entrypoint python fourth-law-agent:v0.10.12-candidate \
  -c 'import compileall; raise SystemExit(0 if compileall.compile_dir("/app/app", quiet=1) else 1)' >>"$REPORT" 2>&1
docker run --rm --entrypoint python \
  -v "$SOURCE:/workspace:ro" -w /workspace fourth-law-agent:v0.10.12-candidate \
  -c 'import unittest; loader=unittest.defaultTestLoader; suite=unittest.TestSuite(); [suite.addTests(loader.discover("tests", pattern=p)) for p in ("test_codex*.py", "test_scoped_verification.py", "test_workspace_memory.py", "test_human_efficiency.py", "test_agent_budget.py", "test_evidence_lineage.py")]; result=unittest.TextTestRunner(verbosity=2).run(suite); raise SystemExit(0 if result.wasSuccessful() else 1)' >>"$REPORT" 2>&1
...
for _ in $(seq 1 60); do
  ...
  echo "$health" | grep -q '"version":"0.10.12"' && break
  sleep 2
done
...
assert project['project']['version'] == '0.10.12'
...
{
  echo FOURTH_LAW_EVIDENCE_LINEAGE_V0_10_12_DEPLOYED
  ...
  echo canonical_screenshot_lineage=true
  echo evidence_endpoint=true
  echo hash_only_evidence=true
...
trap - ERR
echo FOURTH_LAW_EVIDENCE_LINEAGE_V0_10_12_READY

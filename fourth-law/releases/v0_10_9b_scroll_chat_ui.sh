...
REPORT=/tmp/fl-v0109-codex-chat-ui.txt
STAGE="$(mktemp -d /tmp/fl-v0109-stage.XXXXXX)"
BACKUP="$(mktemp -d /opt/fl-v0109-backup.XXXXXX)"
STEP=starting
DEPLOY_STARTED=0
...
fail() {
  ...
  {
    echo FOURTH_LAW_CODEX_CHAT_UI_V0_10_9_FAILED
    echo "step=$STEP"
...
if test -n "$changed"; then
  for required in app/codex_control.py app/static/codex.html tests/test_codex_control.py tests/test_codex_workspace_ui.py; do
    echo "$changed" | grep -qx "$required"
  done
  unexpected="$(echo "$changed" | grep -Ev '^(app/codex_control\.py|app/static/codex\.html|app/main\.py|app/control_room\.py|compose\.yaml|tests/test_codex_control\.py|tests/test_codex_workspace_ui\.py)$' || true)"
  test -z "$unexpected"
else
...
grep -q '"version": "0.10.9"' "$CONTROL/app/codex_control.py"
...
if test -n "$(runuser -u "$DEV_USER" -- git -C "$CONTROL" status --porcelain)"; then
  runuser -u "$DEV_USER" -- git -C "$CONTROL" add \
    app/codex_control.py app/static/codex.html app/main.py app/control_room.py compose.yaml tests/test_codex_control.py tests/test_codex_workspace_ui.py
  runuser -u "$DEV_USER" -- git -C "$CONTROL" \
    -c user.name='Fourth Law Release Gate' -c user.email='release@fourth-law.local' \
    commit -m 'Fix Codex workspace scrolling and direct chat ergonomics' >>"$REPORT" 2>&1
fi
...
docker build --quiet -t fourth-law-agent:v0.10.9-candidate "$STAGE" >>"$REPORT" 2>&1
docker run --rm --entrypoint python fourth-law-agent:v0.10.9-candidate \
  -c 'import compileall; raise SystemExit(0 if compileall.compile_dir("/app/app", quiet=1) else 1)' >>"$REPORT" 2>&1
docker run --rm --entrypoint python \
  -v "$SOURCE:/workspace:ro" -w /workspace fourth-law-agent:v0.10.9-candidate \
  -m unittest discover -s tests -p 'test_codex*.py' -v >>"$REPORT" 2>&1
...
for _ in $(seq 1 60); do
  ...
  echo "$health" | grep -q '"version":"0.10.9"' && break
  sleep 2
done
...
assert project['project']['version'] == '0.10.9'
...
{
  echo FOURTH_LAW_CODEX_CHAT_UI_V0_10_9_DEPLOYED
...
trap - ERR
echo FOURTH_LAW_CODEX_CHAT_UI_V0_10_9_READY

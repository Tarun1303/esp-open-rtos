...
cd "$PROJECT"
docker compose build agent >/tmp/fl0104-build.log 2>&1
docker rm -f fourthlaw-v0104-regression >/dev/null 2>&1 || true
docker run --name fourthlaw-v0104-regression -i --entrypoint python fourth-law-agent-agent:latest - <<'PY'
import tempfile
from pathlib import Path
from app.code_workspace import CodeWorkspace
root=Path(tempfile.mkdtemp()); live=root/'live'; stage=root/'stage'; idx=root/'idx.json'; live.mkdir()
...
PY
docker rm fourthlaw-v0104-regression >/dev/null
'''

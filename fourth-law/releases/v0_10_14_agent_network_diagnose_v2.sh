...
PY
  ...
  systemctl list-units 'fourthlaw*' --all --no-legend --no-pager 2>&1 || true
  systemctl list-timers --all --no-legend --no-pager 2>&1 | grep -E 'fourthlaw|docker|compose' || true
  echo active_release_units_end
  echo relevant_processes_begin
  ps -eo pid,ppid,etimes,stat,args | grep -E 'fourthlaw-release|docker compose|live_chat|sse_hydration' | grep -v grep || true
  echo relevant_processes_end
  echo healthcheck_begin
  docker inspect fourth-law-agent --format 'restart_policy={{json .HostConfig.RestartPolicy}} health_config={{json .Config.Healthcheck}} health_state={{json .State.Health}}' 2>&1 || true
  echo healthcheck_end
  echo docker_events_begin
  docker events --since 15m --until 0s --filter container=fourth-law-agent --filter event=create --filter event=start --filter event=kill --filter event=die --filter event=restart --filter event=destroy --filter event=connect --filter event=disconnect --format '{{.Time}}|{{.Action}}|{{json .Actor.Attributes}}' 2>&1 || true

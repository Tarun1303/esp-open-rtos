...

{
  ...
  docker inspect fourth-law-agent --format 'network_mode={{.HostConfig.NetworkMode}} ports={{json .NetworkSettings.Ports}} networks={{range $name, $cfg := .NetworkSettings.Networks}}{{$name}}={{$cfg.IPAddress}} {{end}}' 2>&1 || true
  echo inspect_end
  echo internal_health_begin
  ...
  echo internal_health_end
  echo host_listener_begin
  ss -ltnp 'sport = :8787' 2>&1 || true
  echo host_listener_end
  echo logs_begin

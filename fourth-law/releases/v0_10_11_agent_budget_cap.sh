...
REPORT=/tmp/fl-v01011-agent-cap.txt
STAGE="$(mktemp -d /tmp/fl-v01011-stage.XXXXXX)"
BACKUP="$(mktemp -d /opt/fl-v01011-backup.XXXXXX)"
STEP=starting
DEPLOY_STARTED=0
...
fail() {
  ...
  {
    echo FOURTH_LAW_EXECUTION_CAP_V0_10_11_FAILED
    echo "step=$STEP"
...
if test -n "$changed"; then
  unexpected="$(echo "$changed" | grep -Ev '^(app/code_workspace\.py|app/codex_control\.py|app/control_room\.py|app/intelligence_engine\.py|app/main\.py|app/problem_engine\.py|app/scoped_verification\.py|app/shared_memory\.py|app/static/control_room\.html|tests/test_agent_budget\.py|tests/test_codex_control\.py|tests/test_codex_workspace_ui\.py|tests/test_scoped_verification\.py|tests/test_workspace_memory\.py|tests/test_human_efficiency\.py)$' || true)"
...
grep -q '"version": "0.10.11"' "$CONTROL/app/codex_control.py"
grep -q 'class ScopedVerificationContract' "$CONTROL/app/scoped_verification.py"
...
if test -n "$(runuser -u "$DEV_USER" -- git -C "$CONTROL" status --porcelain)"; then
  runuser -u "$DEV_USER" -- git -C "$CONTROL" add \
    ...
    tests/test_scoped_verification.py tests/test_workspace_memory.py tests/test_human_efficiency.py tests/test_agent_budget.py
...
docker build --quiet -t fourth-law-agent:v0.10.11-candidate "$STAGE" >>"$REPORT" 2>&1
docker run --rm --entrypoint python fourth-law-agent:v0.10.11-candidate \
  -c 'import compileall; raise SystemExit(0 if compileall.compile_dir("/app/app", quiet=1) else 1)' >>"$REPORT" 2>&1
docker run --rm --entrypoint python \
  -v "$SOURCE:/workspace:ro" -w /workspace fourth-law-agent:v0.10.11-candidate \
  -c 'import unittest; loader=unittest.defaultTestLoader; suite=unittest.TestSuite(); [suite.addTests(loader.discover("tests", pattern=p)) for p in ("test_codex*.py", "test_scoped_verification.py", "test_workspace_memory.py", "test_human_efficiency.py", "test_agent_budget.py")]; result=unittest.TextTestRunner(verbosity=2).run(suite); raise SystemExit(0 if result.wasSuccessful() else 1)' >>"$REPORT" 2>&1
...
for _ in $(seq 1 60); do
  ...
  echo "$health" | grep -q '"version":"0.10.11"' && break
  sleep 2
done
...
assert project['project']['version'] == '0.10.11'
...
{
  echo FOURTH_LAW_EXECUTION_CAP_V0_10_11_DEPLOYED
...
trap - ERR
echo FOURTH_LAW_EXECUTION_CAP_V0_10_11_READY

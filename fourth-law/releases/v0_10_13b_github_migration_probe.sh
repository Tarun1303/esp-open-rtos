...
REPORT=/tmp/fl-v01013b-github-probe.txt
...
fail() {
  ...
  trap - ERR
  {
    echo FOURTH_LAW_GITHUB_MIGRATION_PROBE_V2_FAILED
...
source_branch="$(runuser -u "$DEV_USER" -- git -C "$SOURCE" symbolic-ref --short HEAD)"
source_head="$(runuser -u "$DEV_USER" -- git -C "$SOURCE" rev-parse HEAD)"
source_dirty_count="$(runuser -u "$DEV_USER" -- git -C "$SOURCE" status --porcelain | wc -l | tr -d ' ')"
tracked_count="$(runuser -u "$DEV_USER" -- git -C "$SOURCE" ls-files | wc -l | tr -d ' ')"
...
remote_read=false
if HOME=/root GH_CONFIG_DIR=/root/.config/gh git ls-remote https://github.com/Tarun1303/fourth-law.git >/dev/null 2>>"$REPORT"; then
  remote_read=true
fi

push_dry_run=false
if HOME=/root GH_CONFIG_DIR=/root/.config/gh git -c safe.directory="$SOURCE" -C "$SOURCE" \
  push --dry-run https://github.com/Tarun1303/fourth-law.git main:main >>"$REPORT" 2>&1; then
  push_dry_run=true
fi
...
for role in supervisor architecture runtime control-room execution efficiency; do
  ...
  if test -e "$path/.git"; then
    ...
    branch="$(runuser -u "$DEV_USER" -- git -C "$path" symbolic-ref --short HEAD 2>/dev/null || echo detached)"
    dirty="$(runuser -u "$DEV_USER" -- git -C "$path" status --porcelain | wc -l | tr -d ' ')"
...
{
  echo FOURTH_LAW_GITHUB_MIGRATION_PROBE_V2_READY
...
trap - ERR
echo FOURTH_LAW_GITHUB_MIGRATION_PROBE_V2_DISPATCHED

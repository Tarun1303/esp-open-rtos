...
STEP=sandbox_prerequisite
export DEBIAN_FRONTEND=noninteractive
apt-get update >>"$REPORT" 2>&1
apt-get install -y --no-install-recommends bubblewrap apparmor-profiles apparmor-utils >>"$REPORT" 2>&1
command -v bwrap >>"$REPORT"
if test -f /usr/share/apparmor/extra-profiles/bwrap-userns-restrict; then
  install -m 0644 /usr/share/apparmor/extra-profiles/bwrap-userns-restrict \
    /etc/apparmor.d/bwrap-userns-restrict
  apparmor_parser -r /etc/apparmor.d/bwrap-userns-restrict >>"$REPORT" 2>&1
fi
runuser -u "$DEV_USER" -- bwrap --ro-bind / / true >>"$REPORT" 2>&1
...
runuser -u "$DEV_USER" -- git -C "$SOURCE" status --short --branch >>"$REPORT"
runuser -u "$DEV_USER" -- test -r "$SOURCE/AGENTS.md"
runuser -u "$DEV_USER" -- env HOME="$DEV_HOME" CODEX_HOME="$CODEX_DIR" \

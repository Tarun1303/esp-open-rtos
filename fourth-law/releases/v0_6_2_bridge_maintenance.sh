#!/usr/bin/env bash
set -Eeuo pipefail
MAINT=/opt/fourth-law-agent/bridge-maintenance-v12.sh
cat > "$MAINT" <<'EOS'
#!/usr/bin/env bash
set -Eeuo pipefail
...
HOME=/root GH_CONFIG_DIR=/root/.config/gh gh issue comment 7 --repo Tarun1303/factory --body 'BRIDGE_UPGRADED {"version":"1.2","features":["api_check","job","job_result"]}' >/dev/null 2>&1 || true
EOS
chmod 700 "$MAINT"
systemd-run --unit=fourthlaw-maintenance-v12 --collect --wait /bin/bash "$MAINT"
echo FOURTHLAW_MAINTENANCE_DISPATCHED

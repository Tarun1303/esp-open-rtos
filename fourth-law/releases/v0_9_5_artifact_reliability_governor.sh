...
STAMP="$(date +%s)"
BACKUP="$PROJECT/backups/v0.9.5-artifact-reliability-$STAMP"
mkdir -p "$BACKUP"
...
rollback(){
  ...
  cd "$PROJECT"
  docker compose build agent >/tmp/fl095-rollback-build.log 2>&1 || true
  docker compose up -d --force-recreate agent >/tmp/fl095-rollback-up.log 2>&1 || true
  HOME=/root GH_CONFIG_DIR=/root/.config/gh gh issue comment 7 --repo Tarun1303/factory --body 'ARTIFACT_RELIABILITY_V0_9_5_ROLLED_BACK' >/dev/null 2>&1 || true
  exit 1
}
trap rollback ERR
...
# 1) Stage-aware reasoning/output budget. Hidden reasoning must not consume a tiny
# structured-output ceiling on routine execution/verification calls.
...
if 'reasoning_effort = "none"' not in s:
    if old not in s:
        raise SystemExit('v0.9.5 output-cap anchor missing')
...
if '"reasoning": {"effort": reasoning_effort}' not in s:
    if old not in s:
        raise SystemExit('v0.9.5 model-settings anchor missing')
    s=s.replace(old,new,1)

# 2) Artifact/design/reasoning tasks should validate the produced artifact itself,
# not repeatedly ask another LLM to demand live/deployed evidence on every step.
...
if '"mode": "deterministic-artifact"' not in s:
    if anchor not in s:
        raise SystemExit('v0.9.5 artifact-gate anchor missing')
    s=s.replace(anchor,replacement,1)

# 3) Make execution prompts explicitly concise for artifact-mode work so model output
# remains bounded without sacrificing the useful specification content.
...
if 'keep the result concise and implementation-oriented' not in s:
    if old not in s:
        raise SystemExit('v0.9.5 execute-prompt anchor missing')
    s=s.replace(old,new,1)
...
# Free/local regression: assert the new semantics are present without spending API credits.
python3 - <<'PY'
from pathlib import Path
s=Path('/opt/fourth-law-agent/app/intelligence_engine.py').read_text()
assert 'deterministic-artifact' in s
assert 'artifact_mode = any' in s
assert 'reasoning_effort = "none"' in s
assert 'reasoning_effort = "low"' in s
assert 'output_cap = 1450' in s
assert 'output_cap = 1900' in s
print('V095_LOCAL_RELIABILITY_REGRESSION_OK')
PY

cd "$PROJECT"
docker compose build agent >/tmp/fl095-build.log 2>&1
docker compose up -d --force-recreate agent >/tmp/fl095-up.log 2>&1

...
REPORT=/tmp/fl-v0107b-coding-workspace.txt
STAGE="$(mktemp -d /tmp/fl-v0107b-stage.XXXXXX)"
BACKUP="$(mktemp -d /opt/fl-v0107b-backup.XXXXXX)"
...
fail() {
  ...
  {
    echo CODEX_CODING_WORKSPACE_V0_10_7B_FAILED
    echo "step=$STEP"
...
if ! systemctl is-active --quiet fourthlaw-codex.service; then
  echo "appserver_recovery=restart" >>"$REPORT"
...
echo "candidate_health=$health" >>"$REPORT"
...
DEPLOY_STARTED=0
{
  echo CODEX_CODING_WORKSPACE_V0_10_7B_DEPLOYED
  echo version=0.10.7
...
trap - ERR
echo CODEX_CODING_WORKSPACE_V0_10_7B_READY

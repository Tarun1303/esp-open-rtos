...
for role in supervisor runtime control-room execution efficiency architecture; do
  ...
  if grep -q '"final_answer": "final"' "$repo/app/codex_control.py" \
    && grep -q 'test_final_answer_protocol_phase_reaches_public_transcript' "$repo/tests/test_codex_runtime_contract.py"; then

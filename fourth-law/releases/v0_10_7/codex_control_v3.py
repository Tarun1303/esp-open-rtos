...
ROLE_DETAILS = {
    "runtime": {
        ...
        "description": "Orchestration, lifecycle, continuation and bounded delegation.",
        "owns": ["main.py", "intelligence_engine.py", "problem_engine.py"],
    },
    "control-room": {
        "label": "Control Room",
        "description": "Operator experience, realtime APIs, SSE and intervention flows.",
        "owns": ["control_room.py", "codex_control.py", "app/static"],
    },
    "execution": {
        "label": "Execution safety",
        "description": "Staging, validation, release gates, checksums and rollback.",
        "owns": ["code_workspace.py", "compose.yaml", "release integration"],
    },
    "efficiency": {
        "label": "Efficiency & memory",
        "description": "Context compression, shared memory and request governance.",
        "owns": ["shared_memory.py", "cost and context governors"],
    },
}
MAX_EVENTS = 500
...
class CodexBridge:
    ...
    def public(self, state: dict[str, Any]) -> dict[str, Any]:
        result = {k: state.get(k) for k in (
            "id", "title", "role", "model", "thread_id", "turn_id", "status",
            "created_at", "updated_at", "last_error", "messages", "events", "diff", "plan", "archived"
        )}
        result["role_details"] = ROLE_DETAILS.get(str(state.get("role")), {})
        result["security"] = {
            "filesystem": "selected worktree only",
            "network": "denied for commands",
            "credentials": "denied",
            "production_write": False,
            "deployment": "verified release gate",
        }
        return result
    ...
    async def _connect(self, state: dict[str, Any]) -> Any:
        ...
        await self._request(sid, "initialize", {
            "clientInfo": {"name": "fourth_law_control_room", "title": "Fourth Law Control Room", "version": "0.10.7"}
        })
    ...
    async def create(self, role: str, message: str) -> dict[str, Any]:
        ...
        now = time.time()
        state = {
            "id": sid, "title": message.strip().splitlines()[0][:90],
            "role": role, "model": ROLE_MODELS[role], "thread_id": None,
            "turn_id": None, "status": "starting", "created_at": now, "updated_at": now,
            "last_error": "", "messages": [], "events": [], "diff": "", "plan": [], "archived": False,
        }
        self.write(state)
        try:
            ...
            self.emit(state, "request_failed", state["last_error"])
            if isinstance(exc, HTTPException):
                raise
            raise HTTPException(status_code=502, detail=f"Codex could not start: {str(exc)[:300]}") from exc
        ...
        async with lock:
            try:
                ...
                try:
                    state = self.read(sid)
                    state["last_error"] = str(exc)[:1000]
                    if state.get("status") not in {"working", "interrupted"}:
                        state["status"] = "failed"
                    self.emit(state, "request_failed", state["last_error"])
                except Exception:
                    pass
    ...
    def update(self, sid: str, title: str | None, archived: bool | None) -> dict[str, Any]:
        state = self.read(sid)
        if title is not None:
            clean_title = title.strip()
            if not clean_title:
                raise HTTPException(status_code=422, detail="Title cannot be empty")
            state["title"] = clean_title[:120]
        if archived is not None:
            state["archived"] = bool(archived)
        self.emit(state, "session_updated", "Coding thread details updated")
...
async def list_sessions(fl_session: str | None = Cookie(default=None)):
    ...
    for path in sorted(STATE_DIR.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)[:80]:
        try:
            ...
            row = {k: state.get(k) for k in ("id", "title", "role", "model", "status", "created_at", "updated_at", "last_error", "archived")}
            row["message_count"] = len(state.get("messages") or [])
            rows.append(row)
        except Exception:
            continue
    roles = [{"id": role, "model": ROLE_MODELS[role], **ROLE_DETAILS[role]} for role in ROLE_WORKTREES]
    return {
        "sessions": rows,
        "roles": roles,
        "capabilities": {
            "persistent_threads": True, "active_turn_steering": True,
            "live_events": True, "diffs": True, "plans": True,
            "rename": True, "archive": True,
        },
    }
...
async def send_message(session_id: str, req: MessageCreate, fl_session: str | None = Cookie(default=None)):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Codex request failed: {str(exc)[:300]}") from exc


@router.patch("/control-room/api/codex/sessions/{session_id}")
async def update_session(session_id: str, req: SessionUpdate, fl_session: str | None = Cookie(default=None)):
    _auth(fl_session)
    return bridge.update(session_id, req.title, req.archived)
...
async def interrupt(session_id: str, fl_session: str | None = Cookie(default=None)):
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Codex interrupt failed: {str(exc)[:300]}") from exc

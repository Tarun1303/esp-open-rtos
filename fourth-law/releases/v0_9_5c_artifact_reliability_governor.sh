...
STAMP="$(date +%s)"
BACKUP="$PROJECT/backups/v0.9.5c-artifact-reliability-$STAMP"
mkdir -p "$BACKUP"
...
rollback(){
  ...
  cd "$PROJECT"
  docker compose build agent >/tmp/fl095c-rb-build.log 2>&1 || true
  docker compose up -d --force-recreate agent >/tmp/fl095c-rb-up.log 2>&1 || true
  HOME=/root GH_CONFIG_DIR=/root/.config/gh gh issue comment 7 --repo Tarun1303/factory --body 'ARTIFACT_RELIABILITY_V0_9_5C_ROLLED_BACK' >/dev/null 2>&1 || true
  exit 1
}
trap rollback ERR
python3 - <<'PY'
...
if 'reasoning_effort = "none"' not in s:
    if old not in s: raise SystemExit('v0.9.5c output-cap anchor missing')
    s=s.replace(old,new,1)
needle='                "max_tokens": output_cap,\n'
if '"reasoning": {"effort": reasoning_effort}' not in s:
    if needle not in s: raise SystemExit('v0.9.5c max_tokens line missing')
...
if '"mode": "deterministic-artifact"' not in s:
    if anchor not in s: raise SystemExit('v0.9.5c artifact gate anchor missing')
...
if 'artifact-reliability-v0.9.5' not in s:
    ...
    if marker not in s: raise SystemExit('v0.9.5c architecture marker missing')
...
cd "$PROJECT"
docker compose build agent >/tmp/fl095c-build.log 2>&1
docker compose up -d --force-recreate agent >/tmp/fl095c-up.log 2>&1
ok=0
